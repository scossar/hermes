...
const base: string = (process.argv[2] ?? "http://127.0.0.1:9119").replace(
  /\/$/,
  "",
);

async function fetchSessionToken(base: string) {
...
function toWebSocketUrl(base: string, token: string) {
...
async function main() {
  ...
  // A temporary `onMessage` listener during bootstrap, separate from
  // the permanent one that starts relaying to stdout.
