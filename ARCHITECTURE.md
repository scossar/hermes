...
- `application.lua` composes machine, controller, and runtime and owns public submission/new-session callbacks.
- `chat.lua` and `session.lua` are compatibility facades. Process, RPC, state, history, buffer, activity, approval, and interaction modules are adapters, not lifecycle authorities.
